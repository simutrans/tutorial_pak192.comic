...
class tutorial.chapter_03 extends basic_chapter
{
	...
	//Para el puente
	//------------------------------------------------------------------------------------------
	c_lock1 = [tile_x(92,109,8), tile_x(93,109,8)]
	c_bway_lim2 = {b = coord(95,109), a = coord(90,109)}
	c_brge2 = {a = coord3d(91,109,8), b = coord3d(94,109,8), dir = 5}
			...
			case 2:
				...
				if (pot0==0){
					...
					if (pos.x>=bord1_lim.a.x && pos.y>=bord1_lim.a.y && pos.x<=bord1_lim.b.x && pos.y<=bord1_lim.b.y){
						...
						return all_control(result, gl_wt, gl_st1, way, ribi, tool_id, pos, r_way.c, name)
					}
				...
				//Construye un puente
					...
					if (pos.x>=c_bway_lim2.a.x && pos.y>=c_bway_lim2.a.y && pos.x<=c_bway_lim2.b.x && pos.y<=c_bway_lim2.b.y){
						...
						else return all_control(result, gl_wt, gl_st1, way, ribi, tool_id, pos, r_way.c, name)
					}
					else
				...
				if (pot0==0){
					if (pos.x>=c_way6_lim.a.x && pos.y>=c_way6_lim.a.y && pos.x<=c_way6_lim.b.x && pos.y<=c_way6_lim.b.y){
						if (tool_id==tool_build_way || tool_id == tool_build_bridge || tool_id == tool_build_tunnel){
							return all_control(result, gl_wt, gl_st2, way, ribi, tool_id, pos, r_way.c, name)
						}
					}
	...
	function script_text()
	{
		...
		switch (this.step) {
			...
			case 2:
				...
				//Para el puente
				if (pot1==0){
					local t_start = my_tile(labe_brge)
			...
			case 6:
				...
				if (pot1==0){
					...
					command_x.set_slope(player, t_slope1, slope1.dir)
					if(t_slope2.z < (slope2.c_z))
						command_x.set_slope(player, t_slope2, slope.all_up_slope)					
				}
				//Para el tunel
				if (pot2==0){
					...
					try {
						t.work(player, t_start, sc_tunn1_name)
					}
					catch(ev) {
						return null
					}
			...
			case 8:
				if (pot0==0){
					...
					local t = command_x(tool_build_way)
					t.set_flags(2)	
					local err = t.work(player, coora, coorb, sc_way2_name)
				...
				if (pot1==0){
					...
					t.set_flags(2)
					try {
						t.work(player, t_tunn, sc_tunn2_name)
					}
					catch(ev) {
						return null
					}
				...
				if (pot2==0){
					...
					for(local j = 0; j<(c_list.len());j++){
						...
						t_tun.work(player, t_start, t, sc_tunn2_name)
					...
					t_tun.work(player, c_start, c_end, sc_tunn2_name)
		            ...
		            for(local j=0;j<c_way_lim1.len();j++){
						if(glsw[j] == 0){
							...
							if(list[j].tunn) {
								...
								t.work(player, tile_a, tile_b, sc_tunn2_name)
							}
							else {
								t.set_flags(2)			
								t.work(player, tile_a, tile_b, sc_way2_name)
							}							
						}
		            }
				}
			...
			case 10:
				...
				if (pot2==0){
					...
					local t = command_x(tool_build_depot)
					local err = t.work(player, tile, sc_dep2_name)
				}
				return null
				break
	...
	function set_all_rules(pl) {
		...
		local forbid =	[	4136,4140,4126,4103,4134,4135, tool_add_city,tool_fill_trees,
		...
		switch (this.step) {
			case 1:
				local forbid=	[
									tool_remove_wayobj, tool_build_way, tool_build_bridge, tool_build_tunnel, tool_build_station,
			...
			case 2:
				local forbid = [tool_remove_wayobj, tool_build_station, tool_build_depot, tool_build_roadsign, tool_build_wayobj, tool_build_tunnel]
				foreach (tool_id in forbid)
					rules.forbid_way_tool(pl, tool_id, wt_rail, "")			
				break

			case 3:
				local forbid = [tool_remove_wayobj, tool_build_depot, tool_build_roadsign, tool_build_wayobj, tool_build_tunnel]
				foreach (tool_id in forbid)
					rules.forbid_way_tool(pl, tool_id, wt_rail, "")
				break

			case 4:
				local forbid = [tool_remove_wayobj, tool_build_roadsign, tool_build_wayobj, tool_build_tunnel]
				foreach (tool_id in forbid)
					rules.forbid_way_tool(pl, tool_id, wt_rail, "")
				break
	...
	function is_tool_active(pl, tool_id, wt) {
		...
		switch (this.step) {
			...
			case 5://Schedule
				...
				break
			case 6: 
				local t_list = [tool_build_way, tool_remove_way, tool_remover, tool_build_station, tool_build_bridge, tool_build_tunnel]
				local wt_list = [gl_wt]
				local res = update_tools(t_list, tool_id, wt_list, wt)
				result = res.result
				if(res.ok)  return result
			...
			case 7:
				...
				break	
			case 8:
				local t_list = [-t_icon.narr, tool_remove_way, tool_build_way, tool_build_tunnel]
				local wt_list = [wt_narrowgauge]
				local res = update_tools(t_list, tool_id, wt_list, wt)
				result = res.result
				if(res.ok)  return result
				break	
			case 9:
				local t_list = [-t_icon.narr, -t_icon.slope, -tool_setslope, tool_build_way, tool_build_roadsign, tool_build_tunnel]
				local wt_list = [wt_narrowgauge]
				local res = update_tools(t_list, tool_id, wt_list, wt)
				result = res.result
				if(res.ok)  return result
				break	
			case 10:
				local t_list = [-t_icon.sigd, -t_icon.narr, -t_icon.slope, -tool_setslope, -tool_remover, tool_build_depot, tool_build_wayobj]
				local wt_list = [wt_narrowgauge]
				local res = update_tools(t_list, tool_id, wt_list, wt)
				result = res.result
				if(res.ok)  return result
				break
			case 11://Schedule
				local t_list = [-t_icon.sigd, -t_icon.narr, -t_icon.slope, -tool_setslope, -tool_remover, -t_icon.rail]
				local wt_list = [wt_narrowgauge]
				local res = update_tools(t_list, tool_id, wt_list, wt)
				result = res.result
				if(res.ok)  return result
	...
	function is_tool_allowed(pl, tool_id, wt){
		...
		local gt_list =	[
							t_icon.road, t_icon.ship, t_icon.plane, t_icon.tram,
							t_icon.magl, t_icon.exte, t_icon.wremo, t_icon.other, t_icon.trkplus, 
							t_icon.siga, t_icon.sigb, t_icon.sigc, tool_lower_land, tool_raise_land, tool_restoreslope,
							tool_change_water_height, tool_set_climate, 
						]
		if(step < 6){
		...
		switch (this.step) {
			case 1:
				local t_list = [-tool_remover, -t_icon.narr, 0] // 0 = all tools allowed
				local wt_list = [gl_wt]
				local res = update_tools(t_list, tool_id, wt_list, wt)
				result = res.result
				if(res.ok)  return result
